...
const checkStatus = () => {
  const endpoint = 'http://127.0.0.1:5001/api/v1/status';
...
const searchPlaces = (requestBody) => {
  const endpoint = 'http://127.0.0.1:5001/api/v1/places_search/';
...
const filterAmenities = () => {
  ...
  $('.amenities input[type="checkbox"]').change(function () {
    ...
    if ($(this).is(':checked')) {
      requestBody.amenities.push(id);
    } else {
      const idx = requestBody.amenities.indexOf(id);
      if (idx >= 0) {
        requestBody.amenities.splice(idx, 1);
      }
    }
  });
